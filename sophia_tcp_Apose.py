...
OFFSET: Dict[str, float] = {
    "RightShoulderRoll": deg(65.0),
    "RightElbowPitch": deg(-127.0),
    "LeftShoulderRoll": deg(-65.0),
...
INDEX_MAP: Dict[int, List[ActuatorCmd]] = {
    16: [
        ...
        ActuatorCmd("LeftShoulderRoll", lambda v: -1*_need_vec3(v)[2]),
    ],
    17: [
        ActuatorCmd("RightShoulderPitch", lambda v: _need_vec3(v)[0]),
        ActuatorCmd("RightShoulderRoll", lambda v: -1*_need_vec3(v)[2]),
    ],
    18: [
        ActuatorCmd("LeftShoulderYaw", lambda v: 10*_need_vec3(v)[0]),
    ...
    def _handle(self, conn: socket.socket, addr):
        try:
            ...
            for cmd in INDEX_MAP[idx]:
                ...
                target = clamp(cmd.actuator, delta) # we are sending absolute positions to robot, do not need to consider inital state
                names.append(cmd.actuator)
                values.append(target)
                # print(f"actuator:{cmd.actuator}   val:{target}")
