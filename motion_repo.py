...
All angles in degrees. Unspecified joints default to 0.
"""
...
def get_motion(name: str) -> dict[str, float]:
    """
    Return full joint angles (degrees) for a motion preset.
    Merges preset overrides with defaults (0 for unspecified joints).
    """
    if name not in MOTIONS:
        raise KeyError(f"Unknown motion: {name}")
    angles = DEFAULT_JOINT_ANGLES.copy()
    angles.update(MOTIONS[name])
    return angles
